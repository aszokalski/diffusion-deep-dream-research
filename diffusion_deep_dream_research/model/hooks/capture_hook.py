...
class CaptureHook(BaseHook):
    ...
    _activations: dict[int, torch.Tensor] = PrivateAttr(default=dict)
    ...
    def process_activations(self, activations: torch.Tensor) -> torch.Tensor:
        ...
        transformed_activations = self.activation_transform(activations) if self.activation_transform is not None else activations
