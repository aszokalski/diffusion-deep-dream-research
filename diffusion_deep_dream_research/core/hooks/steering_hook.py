...
class SteeringHookFactory(BaseModel):
    ...
    def create(
        self, *, channel: int, strength: dict[int, float], timesteps: list[int]
    ) -> BaseSteeringHook:
        if self.sae is not None:
            target_device = self.pipe_adapter.pipe.device
            target_dtype = self.pipe_adapter.pipe.unet.dtype

            assert isinstance(self.sae.W_dec, torch.Tensor), (
                "SAE decoder weights have invalid type."
            )

            vector = (
                self.sae.W_dec[channel]
                .detach()
                .clone()
