...
from typing import Callable, ContextManager, Literal, NamedTuple, Optional
...
from diffusion_deep_dream_research.core.hooks.base_hook import (
    BaseHook,
    EarlyExit,
    create_target_hook_context,
)
...
class HookedModelWrapper(nn.Module):
    ...
    @classmethod
    def from_layer(cls, *, pipe: DiffusionPipeline, target_layer_name: str):
        pipe_adapter = ModifiedDiffusionPipelineAdapter(pipe)

        assert hasattr(pipe, "unet"), "UNet model must have named_modules() method."
        assert isinstance(pipe.unet, nn.Module), "UNet must be an instance of nn.Module."
        ...
        capture_hook_factory = CaptureHookFactory(pipe_adapter=pipe_adapter)

        steering_hook_factory = SteeringHookFactory(pipe_adapter=pipe_adapter)
    ...
    @classmethod
    def from_sae(cls, *, pipe: DiffusionPipeline, target_layer_name: str, sae: Sae):
        pipe_adapter = ModifiedDiffusionPipelineAdapter(pipe)
        ...
        capture_hook_factory = CaptureHookFactory(pipe_adapter=pipe_adapter, sae=sae)
        steering_hook_factory = SteeringHookFactory(pipe_adapter=pipe_adapter, sae=sae)
        return cls(
            pipe_adapter=pipe_adapter,
            target_hook_context=target_hook_context,
            capture_hook_factory=capture_hook_factory,
    ...
    def forward(
        ...
        **kwargs,
        ...
        if seeds is not None:
            ...
            generators = [
                torch.Generator(device=self.device).manual_seed(s) for s in expanded_seeds
            ]
    ...
    def forward_with_capture(
        ...
        *,
        ...
        with self.target_hook_context(
            ...
            images = self(
                prompts=prompts, num_images_per_prompt=num_images_per_prompt, output_type="pil"
            )

        assert isinstance(hook, CaptureHook), "Hook is not a CaptureHook instance."
    ...
    def steer(
        ...
        *,
        ...
        with self.target_hook_context(
            ...
            return self(
                prompts=[""] * n_results, seeds=seeds, output_type=output_type, guidance_scale=1.0
            )
    ...
    def decode_latents(self, latents: torch.Tensor) -> np.ndarray:
        ...
        images = self.pipe_adapter.pipe.vae.decode(
            latents.to(self.pipe_adapter.pipe.vae.dtype)
        ).sample
    ...
    def activation(
        ...
        *,
        ...
        """
        with self.target_hook_context(
            self.capture_hook_factory.create(timesteps=[timestep], detach=False, early_exit=True)
        ) as hook:
            batch_size = z.shape[0]
            embeddings = self._empty_embeddings(batch_size)

            try:
                self.pipe_adapter.pipe.unet(z, timestep=timestep, encoder_hidden_states=embeddings)
            except EarlyExit:
                pass
