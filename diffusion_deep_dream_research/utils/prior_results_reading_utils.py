...
class ChannelPriors:
    ...
    def get_latents(
        ...
        :return:
        """
        images_with_latents = self.images_with_latents
        if n_results is not None:
            images_with_latents = images_with_latents[:n_results]
        latents = torch.stack([iwl.latent for iwl in images_with_latents], dim=0)
